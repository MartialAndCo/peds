...
server.addHook('preHandler', async (request, reply) => {
    const urlPath = request.url.split('?')[0];
    if (urlPath === '/status' || urlPath === '/api/status' || urlPath === '/health' || urlPath === '/api/logs/ingest')
        return;
        ...
        bind(ev) {
            ev.on('messages.upsert', (data) => {
                for (const m of data.messages) {
                    ...
                    const existingIdx = list.findIndex((ex) => ex.key.id === m.key.id);
                    if (existingIdx > -1)
                        list[existingIdx] = m;
                    else
                        list.push(m);
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        try {
            ...
            if (remoteJid.includes('@lid')) {
                ...
                if (!resolvedPhoneNumber && msg.key.remoteJidAlt) {
                ...
                if (!resolvedPhoneNumber && msg.key.senderPn) {
...
server.post('/api/sessions/start', async (req, reply) => {
...
// Stop Session
server.post('/api/sessions/stop', async (req, reply) => {
    const { sessionId } = req.body;
    if (!sessionId)
        return reply.code(400).send({ error: 'Missing sessionId' });
...
server.post('/api/sessions/reset', async (req, reply) => {
    const { sessionId } = req.body;
    if (!sessionId)
        return reply.code(400).send({ error: 'Missing sessionId' });
...
server.post('/api/sessions/delete', async (req, reply) => {
    const { sessionId } = req.body;
    if (!sessionId)
        return reply.code(400).send({ error: 'Missing sessionId' });
...
// Typing State
server.post('/api/sendStateTyping', async (req, reply) => {
