...
export default function EditUserPage() {
    ...
    const [password, setPassword] = useState('') // Blank means unchanged
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // 2. Fetch User Data
                // Ideally we'd have a specific GET /api/admin/users/[id], but we can reuse list or add param
                // Let's assume we can fetch by ID or get from list. 
                // Creating a specific GET via query param in the existing route
                const userRes = await fetch(`/api/admin/users?id=${params?.id}`)
                if (!userRes.ok) throw new Error('User not found')

                // Currently GET returns ALL users. We need to filter or update API.
                // Let's verify what GET /api/admin/users does.
                // It returns a list. We can filter client side for now iterate.
                const allUsers = await userRes.json()
                const userData = allUsers.find((u: any) => u.id === params?.id)

                if (!userData) throw new Error('User not found')

                setEmail(userData.email)
                setRole(userData.role)
                setSelectedAgents(userData.agents.map((a: any) => a.id))
        ...
        <div className="max-w-2xl mx-auto space-y-8">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-white/40">Modify permissions and access</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-[#1e293b] p-6 rounded-xl border border-white/5 space-y-4">
                    ...
                    <div className="grid gap-2">
                        <Label className="text-white">Password (Leave blank to keep current)</Label>
                        <Input
                            type="password"
                            className="bg-black/20 border-white/10 text-white"
                            placeholder="••••••••"
