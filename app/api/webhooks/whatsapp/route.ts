...
export async function POST(req: Request) {
    try {
        ...
        if (sourcePhone && normalizedPhone === sourcePhone) {
            ...
            // C. Check for Media Attachment (Fulfillment)
            // If the admin sends an Image or Video, we assume it fulfills the pending request.
            if (payload.type === 'image' || payload.type === 'video' || payload._data?.mimetype?.startsWith('image') || payload._data?.mimetype?.startsWith('video')) {
                console.log('[Webhook] Admin sent media. Attempting to ingest...')
                const { mediaService } = require('@/lib/media')

                // We need the raw media data.
                // WAHA sends 'body' as base64 for media? Or we need to fetch it?
                // Usually payload.body is the base64 data for WAHA if downloadMedia is true, 
                // OR we might need to use `whatsapp.downloadMedia(payload.id)`.
                // Let's assume we need to fetch if body is just a filename/empty.
                // But for now, let's try assuming body or downloading.

                let mediaData = payload.body
                // If body is short (url) or empty, we download.
                if (!mediaData || mediaData.length < 100) {
                    mediaData = await whatsapp.downloadMedia(payload.id)
                }

                const result = await mediaService.ingestMedia(sourcePhone, mediaData, payload._data?.mimetype || payload.type)

                if (result) {
                    await whatsapp.sendText(sourcePhone, `✅ Media sent to ${result.sentTo} (Type: ${result.type})`)
                } else {
                    await whatsapp.sendText(sourcePhone, `⚠️ No pending request found to fulfill. Media saved to bank? (Not implemented yet)`)
                }

                return NextResponse.json({ success: true, handler: 'media_ingest' })
            }
