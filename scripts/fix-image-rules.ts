/**
 * FIX Image Rules - Strict Mode
 * Prevents spontaneous image sending and removes invalid tags
 */
...
async function fixImageRules() {
    console.log('Fixing Image Rules for Anaïs and Lena...')
...
### GESTION DES IMAGES (TRES STRICT)
1. N'envoie JAMAIS d'image spontanément. C'est INTERDIT.
2. Attends TOUJOURS une demande explicite ("envoie une photo").
3. Si on te complimente ("t'es belle"), dis MERCI en TEXTE. PAS D'IMAGE.
4. Si demandé, utilise UNIQUEMENT : [IMAGE:selfie] ou [IMAGE:feet].
5. LE TAG [IMAGE:mirror] EST INTERDIT (ça ne marche pas). N'utilise QUE [IMAGE:selfie].
...
### IMAGE HANDLING (VERY STRICT)
1. NEVER send an image spontaneously. It is FORBIDDEN.
2. ALWAYS wait for an explicit request ("send a pic").
3. If complimented ("you pretty"), say THANKS in TEXT. NO IMAGE.
4. If asked, ONLY use: [IMAGE:selfie] or [IMAGE:feet].
5. THE TAG [IMAGE:mirror] IS BANNED. DO NOT USE IT. Use [IMAGE:selfie] instead.
